    ...
    */
    ...
    SPRITEMANAGER_ITERATE(i, spr) {
        ...
        */
        ...
        if (spr->type == EnemyRockith && movestate == inair && accelY > 0 && nutmeg_death == false) {
            if (CheckCollision(THIS, spr)) {
                SpriteManagerRemove(i);
                PlayFx(CHANNEL_1, 10, 0x4f, 0xC7, 0xF3, 0x73, 0x86);
                isjumping = true;
        ...
        if (spr->type == EnemyBatty && movestate == inair && accelY > 0 && nutmeg_death == false) {
            if (CheckCollision(THIS, spr)) {
                SpriteManagerRemove(i);
                PlayFx(CHANNEL_1, 10, 0x4f, 0xC7, 0xF3, 0x73, 0x86);
                isjumping = true;
        ...
        //kill batty if jump on it
        if (spr->type == EnemyBirdy && movestate == inair && accelY > 0 && nutmeg_death == false) {
            if (CheckCollision(THIS, spr)) {
                SpriteManagerRemove(i);
                PlayFx(CHANNEL_1, 10, 0x4f, 0xC7, 0xF3, 0x73, 0x86);
                isjumping = true;
