...
void Update_StateLevel2() {
	...
	*/
	...
	if (spr_nutmeg1->x > 0 && spr_nutmeg1->x <= 250 && nut_region <= 0) {
		...
		SpriteManagerAdd(EnemyBunny, 31*8, 3*8); //Bunny always 1 higher in y value
	...
	// Up to 94:
		...
		SpriteManagerAdd(EnemyButterfly, 84*8, 12*8);
	...
	// Up to 125:
		...
		SpriteManagerAdd(EnemyButterfly, 114*8, 12*8);
	    nut_region = 3;
	}
	// Up to 156:
	...
	// Up to 188:
		...
		SpriteManagerAdd(SpriteAcorn, 176*8, 7*8);
